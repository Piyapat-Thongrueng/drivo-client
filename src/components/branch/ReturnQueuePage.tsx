...
import { ArrowUpFromLine, Car, Loader2, Search, UserRound } from "lucide-react"
import { useAuth } from "@/contexts/auth-context"
import { getReturnQueue } from "@/lib/api/branch-handover"
import { filterQueue, formatQueueDatetime, isOneWay } from "@/lib/branch-queue"
...
function EmptyState({ search }: { search: string }) {
  return (
    <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-brand-gray-200 bg-brand-gray-50 px-6 py-16 text-center">
      <ArrowUpFromLine className="h-12 w-12 text-brand-gray-300" aria-hidden />
      <div>
        <p className="body-1 font-semibold text-brand-gray-800">
          {search ? "No matching bookings" : "No bookings awaiting return"}
        </p>
        <p className="body-3 mt-1 text-brand-gray-500">
          {search
            ? `No results for "${search}"`
            : "Active bookings scheduled for return at your branch will appear here."}
        </p>
      </div>
    </div>
  )
...
function ReturnQueueCard({ item }: { item: QueueBookingItem }) {
  const oneWay = isOneWay(item)
    ...
    <article className="flex flex-col gap-4 rounded-2xl border border-brand-gray-100 bg-white p-5 shadow-sm transition-shadow hover:shadow-md sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex min-w-0 flex-col gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {oneWay && (
            <span className="body-3 rounded-full bg-amber-100 px-2.5 py-0.5 font-medium text-amber-800">
              One-way
            </span>
          )}
        </div>
        ...
        <div className="body-3 flex flex-col gap-0.5 text-brand-gray-500">
          {oneWay && (
            <span>
              <span className="font-medium text-brand-gray-700">Picked up at: </span>
              {item.pickupBranchName}
            </span>
          )}
          <span>
            <span className="font-medium text-brand-gray-700">Scheduled return: </span>
            {formatQueueDatetime(item.dropoffDatetime)}
          </span>
          <span>
            <span className="font-medium text-brand-gray-700">Pick-up date: </span>
      ...
      <div className="shrink-0">
        <Link
          href={`/branch/bookings/${item.bookingId}/return`}
          className="inline-flex items-center gap-2 rounded-xl bg-brand-gray-900 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-brand-gray-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-gray-900"
        >
...
export default function ReturnQueuePage(): React.JSX.Element {
  ...
  const loadQueue = useCallback(
    async (token: string, q?: string) => {
      ...
      setError(null)
      try {
        const data = await getReturnQueue(token, q)
        setItems(data)
      } catch {
        setError("Unable to load the return queue. Please try again.")
      } finally {
        setLoading(false)
      }
    },
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="heading-5 font-bold text-brand-gray-900">Return Queue</h1>
          <p className="body-3 mt-1 text-brand-gray-500">
            Active bookings waiting for vehicle return at your branch.
          </p>
        </div>
        ...
        <div className="relative w-full max-w-xs">
          ...
          <input
            ...
            aria-label="Search return queue"
          />
        </div>
      </div>
